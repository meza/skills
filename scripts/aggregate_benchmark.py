...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    for eval_idx, eval_dir in enumerate(sorted(search_dir.glob("eval-*"))):
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            # Look for run-N subdirectories (replicated runs layout)
            run_dirs_numbered = sorted(
                [(d, int(d.name.split("-")[1])) for d in config_dir.iterdir()
                 if d.is_dir() and _re.match(r'^run-\d+$', d.name)],
                key=lambda x: x[1]
            )

            # Fall back to flat layout: config dir has grading.json directly
            if not run_dirs_numbered:
                if (config_dir / "grading.json").exists():
                    run_dirs_numbered = [(config_dir, 1)]
                else:
                    continue
