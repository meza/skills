...
try {
  ...
  const source = path.join(pluginRoot, 'claude', 'workflows', 'review-swarm-fast.js')
  const destinationDirectory = path.join(configDir, 'workflows')
  const destination = path.join(destinationDirectory, 'review-swarm-fast.js')
  ...
  if (installedContent !== sourceContent) {
    ...
    process.stderr.write(`[review-swarm-fast] installed workflow -> ${destination}\n`)
  }
} catch (error) {
  // Session startup remains usable because the skill can invoke the bundled path directly.
  process.stderr.write(`[review-swarm-fast] workflow install skipped: ${error && error.message}\n`)
}
